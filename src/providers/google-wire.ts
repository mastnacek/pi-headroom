...
export function isThinkingPart(part: GeminiPart): boolean {
	...
	const isOmpThought =
		Boolean(part.thoughtSignature) && !part.functionCall && !part.functionResponse && !part.inlineData && !part.fileData;
	return isOmpThought;
}
...
export function mapStopReasonString(
	...
	if (upper === "SAFETY" || upper === "RECITATION" || upper === "BLOCKLIST") return "error";
...
function sanitizeSurrogates(text: string): string {
	...
	return sanitized.replace(/\u0000(\d+)\u0000/g, (_, index) => placeholders[Number(index)] ?? "\uFFFD");
}

type AnyContent = { type: string; [k: string]: unknown };

function isImageContent(b: AnyContent | { type: string }): boolean {
	return b.type === "image" && "data" in b && typeof b.data === "string" && "mimeType" in b && typeof b.mimeType === "string";
}
...
export function transformMessages(messages: Message[], model: ModelWire, normalizeToolCallId?: (id: string) => string): Message[] {
	...
	const transformed = imageAware.map((msg) => {
		...
		if (msg.role === "assistant") {
			...
			const contentBlocks = (Array.isArray(assistantMsg.content) ? assistantMsg.content : []) as unknown as AnyContent[];
			...
			return { ...assistantMsg, content: transformedContent as unknown as (typeof assistantMsg.content) };
		}
		return msg;
	});
	...
	for (const msg of transformed) {
		if (msg.role === "assistant") {
			...
			const rawContent = (Array.isArray(assistantMsg.content) ? assistantMsg.content : []) as unknown as AnyContent[];
			// SAFETY: content array is filtered to toolCall blocks
			const toolCalls = rawContent.filter((b) => b.type === "toolCall") as unknown as ToolCall[];
			...
			existingToolResultIds.add((msg as Extract<Message, { role: "toolResult" }>).toolCallId);
			result.push(msg as Message);
...
function resolveThoughtSignature(isSame: boolean, raw: string | undefined): string | undefined {
...
export function convertMessages(model: ModelWire, context: Context): GeminiContent[] {
	...
	const isSameProviderAndModel = (msg: Message) =>
		msg.role === "assistant" && msg.provider === model.provider && msg.api === model.api && msg.model === model.id;

	for (const message of transformed) {
		if (message.role === "user") {
			const parts: GeminiPart[] = [];
			if (typeof message.content === "string") {
				if (message.content.length > 0) parts.push({ text: sanitizeSurrogates(message.content) });
			} else if (Array.isArray(message.content)) {
				for (const block of message.content) {
					if (block.type === "text" && block.text) parts.push({ text: sanitizeSurrogates(block.text) });
					else if (block.type === "image") parts.push({ inlineData: { mimeType: block.mimeType, data: block.data } });
				}
			}
			...
			for (const block of blocks) {
				if (block.type === "text") {
					...
					const thoughtSignature = resolveThoughtSignature(isSame, block.textSignature);
					...
					const thoughtSignature = resolveThoughtSignature(isSame, thinking.thinkingSignature);
					...
					// SAFETY: block type narrowed to toolCall
					const toolCall = block as unknown as ToolCall;
					const thoughtSignature = resolveThoughtSignature(isSameProviderAndModel(message), toolCall.thoughtSignature);
					parts.push({
						functionCall: {
							name: toolCall.name,
			...
			if (typeof message.content === "string") {
				...
				for (const c of message.content) {
					...
					else if (c.type === "image") images.push({ mimeType: c.mimeType, data: c.data });
				}
			}
